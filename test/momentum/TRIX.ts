...
Deno.test('TRIX', function() {
  "use strict";
  Deno.test('should calculate TRIX using the calculate method', function() {
    assertEquals(TRIX.calculate(input), expectResult, 'Wrong Results');
  });

  Deno.test('should be able to calculate TRIX by using getResult', function() {
    let trix = new TRIX(input);
    assertEquals(trix.getResult(),  expectResult, 'Wrong Results while calculating next bar');
  });

  Deno.test('should be able to get TRIX for the next bar using nextValue', function() {
  ...
  Deno.test('should be able to calculate TRIX for reversed input by using calculate method', function() {
    let myInput = Object.assign({}, input);
    myInput.reversedInput = true;
    myInput.values.reverse();
    assertEquals(TRIX.calculate(myInput),  expectResult.slice().reverse(), 'Wrong Results while calculating next bar');
  });
