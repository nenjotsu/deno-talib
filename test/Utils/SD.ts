...
Deno.test('Standard Deviation', function() {
  ...
  Deno.test('should calculate SD using the calculate method', function() {
    assertEquals(SD.calculate({period : period, values : data}), expectResult, 'Wrong Results');
  });

  Deno.test('should be able to calculate EMA by using getResult', function() {
    let sd = new SD({period : period, values : data});
    assertEquals(sd.getResult(),  expectResult, 'Wrong Results while calculating next bar');
  });
  ...
  Deno.test('should be able to calculate ROC for reversed input by using calculate method', function() {
    ...
    assertEquals(SD.calculate(myInput),  expectResult.slice().reverse(), 'Wrong Results while calculating next bar');
  });
